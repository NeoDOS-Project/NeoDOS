...
| 4 | Kernel self-tests (665 tests), netd kthread spawn, benchmarks, NeoInit launch (PID 1, Ring 3) | `:513-663` |
...
| `KERNEL_VERSION_CODE` | `(10 << 8) \| 5` = `0x0A05` | v0.10.5 — **stale in `src/main.rs:82`; actual project version is v0.50.2 (0x3202). Code is truth; doc reflects code** |
| `BOOT_VERSION` | `(10 << 8) \| 5` | Must match kernel version code (currently identical, so no warning) |

The bootloader version (`boot_info.version`) is compared against
`KERNEL_VERSION_CODE` at kernel entry (`src/main.rs:132`). A mismatch is non-fatal but produces
a `kwarn!(Kernel, "Version mismatch...")` via serial. Both values are generated by the build
pipeline (`neodev build` / `neodos-bootloader`). The stale `KERNEL_VERSION_CODE` (0x0A05 vs expected
0x3202 for v0.50.2) is a known doc/code drift to be fixed alongside the version bump — see `CHANGELOG.md`.
