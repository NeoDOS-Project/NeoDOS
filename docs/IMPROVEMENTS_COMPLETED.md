...
## HN. Hostname Infrastructure [COMPLETED v0.50.2]

- [x] **HN. System hostname persistence with Registry** | Files: `neodos-kernel/src/syscall/ob.rs`, `neodos-kernel/src/object/types.rs`, `scripts/gen_system_hiv.py`, `libneodos/src/syscall.rs`, `libnet-nxl/src/main.rs`, `libnet/src/lib.rs`, `userbin/hostname/`, `userbin/ipconfig/src/main.rs`
  - **Registry:** `\Registry\Machine\System\CurrentControlSet\Control\ComputerName\ComputerName = "NeoDOS-PC"` (REG_SZ). Generated by `gen_system_hiv.py`, persisted in SYSTEM hive (NEOHv1, cells 55-56).
  - **ObInfoClass::Hostname = 38:** Queries hostname from Registry. Returns null-terminated string. Works on any Ob object handle.
  - **ObSetInfoClass::SetHostname = 49:** Sets hostname (admin-only). Validates length ≤ 63, creates key path if needed.
  - **libneodos:** `sys_get_hostname(buf)`, `sys_set_hostname(name)` wrappers via `sys_ob_query_info`/`sys_ob_set_info`.
  - **libnet-nxl:** `net_get_hostname(buf, len)` implemented (signature changed from `fn(u32)->u32` placeholder to `fn(*mut u8, u32)->u32`).
  - **userbin/hostname:** New `.NXE` binary. Displays hostname from kernel API. NLT i18n (en-US, es-ES, ca-ES).
  - **Ipconfig:** Now uses `sys_get_hostname()` — unified source of truth. Removed hardcoded `"NeoDOS-PC"`. All infinite-loop error exits → `sys_exit(0)`.
  - **Docs:** `docs/objects.md`, `docs/registry.md`, `CHANGELOG.md`.
  - **Future:** HN-01..HN-05 in `docs/IMPROVEMENTS.md` (dynamic hostname, DHCP Option 12, DNS update, NetBIOS, validation).
  - **Tests:** 665 kernel tests pass. Build verified with `neodev build --image`.
