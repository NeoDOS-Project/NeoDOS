...
const APP_NAME: &str = "loadnem";
const IDS_LOADING: u32 = 1005;
const IDS_LOADED: u32 = 1006;
const IDS_UNLOADING: u32 = 1007;
const IDS_UNLOADED: u32 = 1008;
const IDS_ERR_OPEN: u32 = 1009;
const IDS_ERR_LOAD: u32 = 1010;
const IDS_ERR_UNLOAD: u32 = 1011;
...
fn cmd_load(path: &[u8]) {
    ...
    write_str(b"\r\n");
    write_str(tr_id!(IDS_LOADING).as_bytes());
    ...
    match syscall::sys_ob_create(ob_path, libneodos::syscall::ob_type::DRIVER, None, 0) {
        Ok(fd) => {
            write_str(tr_id!(IDS_LOADED).as_bytes());
            write_str(b", fd=");
            write_u32(fd as u32);
            write_str(b"\r\n");
        }
        Err(e) => {
            write_err(tr_id!(IDS_ERR_LOAD).as_bytes());
            write_err(b" (error ");
            write_u32((-e) as u32);
            write_err(b")\r\n");
        }
...
fn cmd_unload(name: &[u8]) {
    ...
    write_str(b"\r\n");
    write_str(tr_id!(IDS_UNLOADING).as_bytes());
    write_str(name_str.as_bytes());
    write_str(b"...\r\n");

    match syscall::sys_driver_unload(name_str, false) {
        Ok(()) => {
            write_str(tr_id!(IDS_UNLOADED).as_bytes());
            write_str(b"\r\n");
        }
        Err(e) => {
            write_err(tr_id!(IDS_ERR_UNLOAD).as_bytes());
            write_err(b" (error ");
            write_u32((-e) as u32);
            write_err(b")\r\n");
        }
