...
// ── Public utilities (used by completion.rs) ─────

pub fn make_ascii_uppercase(buf: &mut [u8]) {
...
impl Shell {
    ...
    fn env_set(&mut self, key: &[u8], val: &[u8]) {
        ...
        for i in 0..self.env_count {
            if self.env[i].key_len == kl && &self.env[i].key[..kl] == key {
                self.env[i].val[..vl].copy_from_slice(&val[..vl]); self.env[i].val_len = vl; return;
            }
        }
    ...
    fn prompt(&mut self) {
        ...
        match syscall::sys_getcwd(&mut cwdb) {
            ...
            _ => {
                let fallback = b"C:\\";
                let sl = fallback.len().min(384 - p);
                out[p..p + sl].copy_from_slice(&fallback[..sl]); p += sl;
            }
        }
        let suffix = b"> ";
        let sl = suffix.len().min(384 - p);
        out[p..p + sl].copy_from_slice(&suffix[..sl]); p += sl;
    ...
    fn execute_line(&mut self, line: &[u8]) {
        ...
        if pipeline::parse_pipeline(trimmed, &mut pp) > 0 {
