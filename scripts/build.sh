...
if [ "$BUILD_NEODOS_IMAGE" = true ] && command -v python3 >/dev/null 2>&1; then
    ...
    for pkg in neoshell neoinit coredir cd corehelp cpuinfo datetime ver mem vol echo kobj coretype tree corecls corecopy coredel coreren coremd corerd cmdtest; do
