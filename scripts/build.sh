...
if [ "$BUILD_FS_IMAGE" = true ] && command -v python3 >/dev/null 2>&1; then
    ...
    for pkg in neoshell neoinit coredir cd corehelp cpuinfo datetime ver neomem vol echo label coretype tree corecls corecopy coredel coreren coremd corerd cmdtest shtest drives ps keyb kill pri fsck ndreg loadnem progress neotop dhcpd netcfg ipconfig poweroff reboot colors; do
    ...
    echo "[+] Compiling NEM v3 standalone driver (e1000)..."
    DRV_DIR="$PROJECT_ROOT/drivers/e1000"
    if [ -f "$DRV_DIR/build_nem.py" ]; then
        mkdir -p "$NEM_DIR/SYSTEM"
        python3 "$DRV_DIR/build_nem.py" "$NEM_DIR/SYSTEM"
        echo "[✓] e1000.nem compiled"
    else
        echo "[!] e1000 build script not found"
    fi

    echo "[+] Compiling NEM v3 standalone driver (virtio-blk)..."
    DRV_DIR="$PROJECT_ROOT/drivers/virtio-blk"
    if [ -f "$DRV_DIR/build_nem.py" ]; then
        mkdir -p "$NEM_DIR/SYSTEM"
        python3 "$DRV_DIR/build_nem.py" "$NEM_DIR/SYSTEM"
        echo "[✓] virtio-blk.nem compiled"
    else
        echo "[!] virtio-blk build script not found"
    fi
    export NEM_DIR
fi
