...
if [ "$BUILD_FS_IMAGE" = true ] && command -v python3 >/dev/null 2>&1; then
    ...
    for pkg in neoshell neoinit coredir cd corehelp cpuinfo datetime ver neomem vol echo label coretype tree corecls corecopy coredel coreren coremd corerd cmdtest shtest drives ps keyb kill pri fsck ndreg loadnem progress neotop dhcpd netcfg ipconfig poweroff colors; do
