...
''')
    ...
    if args.minimal:
        ...
        inode_system_hiv = allocator.alloc("SYSTEM.HIV")
        ...
        # Read SYSTEM.HIV data (generated by gen_system_hiv.py)
        hiv_path = Path(__file__).parent / "system.hiv"
        if hiv_path.exists():
            system_hiv_data = hiv_path.read_bytes()
            print(f"[*] Read SYSTEM.HIV ({len(system_hiv_data)} bytes)")
        else:
            system_hiv_data = b""
            print("[!] system.hiv not found — Registry directory will be empty at build")
        reg_dir_blocks    = alloc_blocks(inode_registry, 512)   # Registry dir (2 slots)
        system_hiv_blocks = alloc_blocks(inode_system_hiv, len(system_hiv_data))
        ...
        inodes_data = {
            ...
            inode_registry: (dir_mode, 512, pad_blocks(reg_dir_blocks)),
            inode_system_hiv: (MODE_FILE | default_perms_for_filename("SYSTEM.HIV"), len(system_hiv_data), pad_blocks(system_hiv_blocks)),
        ...
        # System\Registry\ directory (SYSTEM.HIV for offline tools, populated at runtime)
        print("[*] Writing System\\Registry directory...")
        blk = reg_dir_blocks[0]
        offset = (DATA_START_SECTOR + blk * 8) * 512
        if system_hiv_data:
            image[offset:offset+256] = create_dir_entry(inode_system_hiv, 1, "SYSTEM.HIV")
            # Write hive content
            print(f"[*] Writing SYSTEM.HIV content ({len(system_hiv_data)} bytes)...")
            for bi, blk_num in enumerate(system_hiv_blocks):
                chunk = system_hiv_data[bi * BLOCK_SIZE:(bi + 1) * BLOCK_SIZE]
                if not chunk:
                    break
                off = (DATA_START_SECTOR + blk_num * 8) * 512
                image[off:off+len(chunk)] = chunk
