...
- **Hostname infrastructure** — Full system hostname implementation with Registry persistence, kernel API, user-library wrappers, and user-facing commands.
  - **Registry**: `\Registry\Machine\System\CurrentControlSet\Control\ComputerName\ComputerName = "NeoDOS-PC"` (REG_SZ, null-terminated). Generated by `scripts/gen_system_hiv.py` at boot, persisted via NEOHv1 hive.
  - **ObInfoClass::Hostname = 38** — Queries the hostname from the kernel. Reads from Registry, returns null-terminated string. Works on any Ob object handle.
  - **ObSetInfoClass::SetHostname = 49** — Sets the hostname in the Registry (admin-only). Validates max length (63 bytes), creates ComputerName key path if missing.
  - **libneodos wrappers**: `sys_get_hostname(buf) -> Result<usize>` via `sys_ob_query_info`, `sys_set_hostname(name) -> Result<()>` via `sys_ob_set_info`.
  - **`userbin/hostname/`** — New `.NXE` binary. Displays the system hostname. i18n via NLT locales (en-US, es-ES, ca-ES).
  - **Ipconfig integration** — `ipconfig` now reads hostname via `sys_get_hostname()` instead of hardcoded `"NeoDOS-PC"`. All infinite-loop error exits replaced with `sys_exit(0)`.
  - **libnet-nxl**: `net_get_hostname(buf, len)` implemented (signature changed from `fn(u32)->u32` placeholder).
  - **NLT locales** — `hostname.toml` and `.nlt` in 3 languages: en-US, es-ES, ca-ES.
  - **Docs**: `docs/objects.md` (+Hostname/SetHostname classes), `docs/registry.md` (+ComputerName path), `docs/IMPROVEMENTS.md` (+5 future improvements: HN-01..HN-05).
